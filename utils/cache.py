        ...
        if datetime.now() - file_mtime < timedelta(days=CACHE_VALIDITY_DAYS):
            try:
                with open(cache_file, 'r') as file:
                    ...
                    region_data = all_data.get(region, None)

                if region_data:
                    cache_timestamp = datetime.fromisoformat(region_data["timestamp"])
                    if datetime.now() - cache_timestamp < timedelta(days=CACHE_VALIDITY_DAYS):
                        return region_data["data"]
                    else:
                        # Cache expired for the region
                        del all_data[region]
